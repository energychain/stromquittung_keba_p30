...
module.exports = function(config) {
  ...
  let meters2 = [];
  const _getLastFinishedSession = function(DEVICE_IP,storage) {
    return new Promise( async function (resolve, reject)  {
        let prep = {};
        let retrier = -1;
        const client = udp.createSocket('udp4');
  ...
  this.quittung = async function(query,storage) {
    let parent = this;
    return new Promise( async function (resolve, reject)  {
      _getLastFinishedSession(query,storage).then(async function(quittung) {
        resolve(quittung);
      })
    });
